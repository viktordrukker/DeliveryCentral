...
function buildStub(
  rows: FakeBudget[],
  yearGroupCounts?: Array<{ fiscalYear: number; count: number }> | null,
): PrismaService {
  const projectBudget = {
    findMany: async (_q: unknown) => rows,
    // Service picks the densest year (most rows) and ties break on the
    // higher fiscalYear. Stub returns the pre-sorted top result.
    groupBy: async (_q: unknown) => {
      if (!yearGroupCounts || yearGroupCounts.length === 0) return [];
      const sorted = [...yearGroupCounts].sort(
        (a, b) => b.count - a.count || b.fiscalYear - a.fiscalYear,
      );
      return sorted.slice(0, 1).map((g) => ({ fiscalYear: g.fiscalYear, _count: { _all: g.count } }));
    },
  };
...
describe('PortfolioFinanceSummaryService', () => {
  ...
  it('defaults to the densest fiscal year with budget rows when none provided', async () => {
    const svc = new PortfolioFinanceSummaryService(
      buildStub([], [
        { fiscalYear: 2024, count: 5 },
        { fiscalYear: 2025, count: 12 }, // densest
        { fiscalYear: 2026, count: 2 },
      ]),
    );
  ...
  it('breaks ties on density by picking the higher fiscal year', async () => {
    const svc = new PortfolioFinanceSummaryService(
      buildStub([], [
        { fiscalYear: 2024, count: 5 },
        { fiscalYear: 2025, count: 5 }, // tie
      ]),
    );
    const r = await svc.summarize();
    expect(r.fiscalYear).toBe(2025);
