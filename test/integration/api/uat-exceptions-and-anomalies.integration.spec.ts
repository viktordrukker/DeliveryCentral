...
describe.skip('API integration: UAT staffing anomalies and exclusions', () => {
  ...
  it('runs the anomaly pack across staffing exclusions, visibility changes, exception surfacing, and degraded integration behavior', async () => {
    const client = createApiTestClient(app);
    const result = await runUatStaffingAnomaliesScenario(client);

    expect(result).toEqual(
      expect.objectContaining({
        activeEmployeeId: expect.any(String),
        anomalyProjectId: expect.any(String),
        bulkProjectId: expect.any(String),
        closureConflictExceptionId: expect.any(String),
      }),
    );
  });
});
