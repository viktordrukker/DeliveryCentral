 ...
 *   conflictingAssignmentIds   — active ProjectAssignment ids overlapping
 *                                the range (allocationPercent > 0).
...
export class LeaveImpactPreviewService {
  ...
  public async preview(input: LeavePreviewInput): Promise<LeaveImpactPreviewDto> {
    ...
    const conflictingAssignmentIds = await this.findConflictingAssignmentIds(
      input.personId,
      start,
      end,
    );
  ...
  private async findConflictingAssignmentIds(
    personId: string,
    start: Date,
    end: Date,
  ): Promise<string[]> {
    const rows = await this.prisma.projectAssignment.findMany({
      where: {
        ...
        AND: [
          ...
          { OR: [{ allocationPercent: null }, { allocationPercent: { gt: 0 } }] },
        ],
        archivedAt: null,
      },
    ...
    return rows
      .filter((r) => r.allocationPercent == null || Number(r.allocationPercent) > 0)
      .map((r) => r.id);
  }
