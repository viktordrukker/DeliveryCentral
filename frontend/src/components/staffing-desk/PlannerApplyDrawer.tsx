...
export function PlannerApplyDrawer({ onApply, onClose, open, simulation, budgetDelta }: Props): JSX.Element | null {
  ...
  // Initialize all as checked when drawer opens
    ...
    <div style={OVERLAY}>
      ...
      <div style={PANEL}>
        <div style={{ padding: 'var(--space-3) var(--space-4)', borderBottom: '1px solid var(--color-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontSize: 15, fontWeight: 600 }}>Review & Apply Plan</div>
        ...
        <div style={{ padding: 'var(--space-3) var(--space-4)', borderTop: '1px solid var(--color-border)', display: 'flex', justifyContent: 'space-between' }}>
          ...
          <button className="button" onClick={handleApply} type="button">
            Apply All ({checkedDispatches.size + checkedHires.size + checkedExtensions.size} actions)
          </button>
        </div>
      </div>
    </div>
