  ...
  return (
    <div>
      <div style={S_TOOLBAR}>
        <div style={S_TABS}>
          <button
            ...
            style={activeTab === 'assignments' ? S_TAB_ACTIVE : S_TAB}
          ...
          <button
            ...
            style={activeTab === 'positions' ? S_TAB_ACTIVE : S_TAB}
      ...
      <div style={{ overflowX: 'auto' }}>
        <table style={S_TABLE} data-testid="workflow-table">
          <colgroup>
            {visibleCols.map((c) => (
          ...
          <tbody>
            {currentItems.length === 0 ? (
              ...
              currentItems.map((row) => (
                <RowRenderer key={row.id} row={row} visibleCols={visibleCols} onRowClick={onRowClick} />
              ))
            )}
          </tbody>
        </table>
...
function RowRenderer({ row, visibleCols, onRowClick }: {
  onRowClick?: (item: StaffingDeskRow) => void;
  row: StaffingDeskRow;
  visibleCols: ColDef[];
}): JSX.Element {
