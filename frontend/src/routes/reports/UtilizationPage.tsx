...
export function UtilizationPage(): JSX.Element {
  ...
  return (
    <PageContainer viewport>
      ...
      {!isLoading && !error && chartData.length > 0 ? (
        <SectionCard title="Utilization by Person — Overview">
          ...
          <div style={{ display: 'flex', fontSize: '11px', gap: '12px', marginTop: '8px' }}>
            {[
              { color: '#bbf7d0', label: '0–50% Underutilized' },
              { color: '#22c55e', label: '51–80% Healthy' },
              { color: '#f59e0b', label: '81–100% Full' },
              { color: '#f97316', label: '101–120% Warning' },
              { color: '#ef4444', label: '121%+ Critical' },
            ].map((item) => (
      ...
      <ViewportTable>
        {isLoading ? <TableSkeleton cols={5} rows={8} /> : null}
