...
describe('W4-06 — StaffingDeskPage clarifies Create-Position button copy', () => {
  const src = readFileSync('src/routes/staffing-desk/StaffingDeskPage.tsx', 'utf-8');

  it('primary CTA is labelled "+ New Position (Quick)"', () => {
    expect(src).toMatch(/\+ New Position \(Quick\)/);
  });

  it('secondary CTA is labelled "New Position (Full page)"', () => {
    expect(src).toMatch(/New Position \(Full page\)/);
  });
