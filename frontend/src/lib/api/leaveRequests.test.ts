...
describe('cancelLeaveRequest', () => {
  ...
  it('issues POST /leave-requests/:id/cancel with an empty body and returns the updated request', async () => {
    ...
    httpPost.mockResolvedValueOnce(dto);

    const result = await cancelLeaveRequest('lr-1');

    expect(httpPost).toHaveBeenCalledWith('/leave-requests/lr-1/cancel', {});
