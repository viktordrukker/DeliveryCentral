...
export const routeManifest: RouteManifestEntry[] = [
  ...
  { allowedRoles: ADMIN_ROLES, description: 'Configure scoring thresholds for the 16-axis project radiator.', group: 'admin-config', navVisible: true, path: '/admin/radiator-thresholds', title: 'Radiator Thresholds' },
  { allowedRoles: ADMIN_ROLES, description: 'Organization-wide reporting cadence, exception thresholds, governance and risk cadence.', group: 'admin-config', navVisible: true, path: '/admin/organization-config', title: 'Organization Config' },
  ...
  { allowedRoles: STAFFING_DESK_ROLES, description: 'Authoritative staffing assignments.', group: 'staffing', navVisible: true, path: '/assignments', title: 'Assignments' },
  ...
  { allowedRoles: MANAGEMENT_ROLES, description: 'Pending approval queue for proposal slates and director sign-off.', group: 'staffing', navVisible: true, path: '/assignments/queue', title: 'Approval Queue' },
  ...
  { allowedRoles: ALL_ROLES, description: 'Monthly timesheet, leave requests, time gaps, and summary.', group: 'time', navVisible: true, path: '/my-time', title: 'My Time' },
  { allowedRoles: TIMESHEET_MANAGER_ROLES, description: 'Approve timesheets and leave, review compliance, manage overtime.', group: 'time', navVisible: true, path: '/time-management', title: 'Time Management' },
  ...
  { allowedRoles: TIMESHEET_MANAGER_ROLES, description: 'Time analytics: hours breakdown by project, person, department with overtime, bench, and utilization.', group: 'reports', navVisible: true, path: '/reports/time', title: 'Time Analytics' },
  { allowedRoles: CAPITALISATION_ROLES, description: 'CAPEX/OPEX capitalisation breakdown for approved timesheets.', group: 'reports', navVisible: true, path: '/reports/capitalisation', title: 'Capitalisation' },
  { allowedRoles: EXPORT_CENTRE_ROLES, description: 'Generate and download XLSX reports for headcount, assignments, timesheets, capitalisation, and workload.', group: 'reports', navVisible: true, path: '/reports/export', title: 'Export Centre' },
  { allowedRoles: EXCEPTIONS_ROLES, description: 'Available vs assigned vs actual hours per person — utilization drill-down.', group: 'reports', navVisible: true, path: '/reports/utilization', title: 'Utilization' },
  { allowedRoles: EXCEPTIONS_ROLES, description: 'Build custom reports from any data source, save templates, and export to XLSX.', group: 'reports', navVisible: true, path: '/reports/builder', title: 'Report Builder' },
  { allowedRoles: ALL_ROLES, description: 'Onboarding and operational case workflows.', group: 'time', navVisible: true, path: '/cases', title: 'Cases' },
  ...
  { allowedRoles: STAFFING_DESK_ROLES, description: 'Unified staffing operations console — assignments, requests, supply-demand gap, timeline, and export.', group: 'staffing', navVisible: true, path: '/staffing-desk', title: 'Staffing Desk' },
  ...
  { allowedRoles: HR_DIRECTOR_ADMIN_ROLES, description: 'Business-action audit trail for investigation and governance workflows.', group: 'admin-governance', navVisible: true, path: '/admin/audit', title: 'Business Audit' },
  { allowedRoles: DIRECTOR_ADMIN_ROLES, description: 'Notification channel and template management.', group: 'admin-governance', navVisible: true, path: '/admin/notifications', title: 'Admin Notifications' },
  { allowedRoles: ADMIN_ONLY_ROLES, description: 'External provider health and synchronization.', group: 'admin-integrations', navVisible: true, path: '/admin/integrations', title: 'Admin Integrations' },
  { allowedRoles: ADMIN_ONLY_ROLES, description: 'Uniform registry of every adapter (Jira, M365, RADIUS, JSM, LDAP, LLM) with status and last-sync.', group: 'admin-integrations', navVisible: true, path: '/admin/integrations/registry', title: 'Integrations Registry' },
  { allowedRoles: ADMIN_ONLY_ROLES, description: 'Read-only health, readiness, and diagnostics visibility.', group: 'admin-integrations', navVisible: true, path: '/admin/monitoring', title: 'Admin Monitoring' },
  { allowedRoles: DIRECTOR_ADMIN_ROLES, description: 'External provider health and synchronization.', group: 'admin-integrations', navVisible: true, path: '/integrations', title: 'Integrations' },
  ...
  { allowedRoles: HR_DIRECTOR_ADMIN_ROLES, description: 'Metadata, validation, and administrative configuration (unified dictionary admin per D-101).', group: 'admin-config', navVisible: true, path: '/metadata-admin', title: 'Metadata / Admin' },
  { allowedRoles: ADMIN_ROLES, description: 'Configure platform-wide behaviour: timesheets, capitalisation, pulse, notifications, and security.', group: 'admin-config', navVisible: true, path: '/admin/settings', title: 'Platform Settings' },
  ...
  { allowedRoles: ADMIN_ROLES, description: 'Lock past time periods so timesheets, expenses, and capitalisation rows can’t be edited retroactively.', group: 'admin-governance', navVisible: true, path: '/admin/period-locks', title: 'Period Locks' },
  // F-13.7 / D-117 — post-install control surface; pairs with /setup install wizard.
  { allowedRoles: ADMIN_ROLES, description: 'Re-run setup steps, inspect install state, and reach diagnostic surfaces.', group: 'admin-config', navVisible: true, path: '/admin/setup', title: 'Setup Operations' },
  ...
  { allowedRoles: ADMIN_ROLES, description: 'Toggle individual features per tenant; view maturity, ownership, and dependency metadata for every registered flag.', group: 'admin-config', navVisible: true, path: '/admin/feature-flags', title: 'Feature Flags' },
  { allowedRoles: HR_DIRECTOR_ADMIN_ROLES, description: 'Bulk import people from a CSV file — up to 200+ records at once.', group: 'admin-config', navVisible: true, path: '/admin/people/import', title: 'Bulk Import' },
  { allowedRoles: ADMIN_ROLES, description: 'Manage external vendors and subcontractors for project staffing.', group: 'admin-config', navVisible: true, path: '/admin/vendors', title: 'Vendors' },
  { allowedRoles: ADMIN_ROLES, description: 'Manage outbound webhook subscriptions with HMAC-SHA256 signed delivery.', group: 'admin-integrations', navVisible: true, path: '/admin/webhooks', title: 'Webhooks' },
  { allowedRoles: ADMIN_ROLES, description: 'Configure BambooHR or Workday HRIS integration for automated employee sync.', group: 'admin-integrations', navVisible: true, path: '/admin/hris', title: 'HRIS Integration' },
  { allowedRoles: ADMIN_ROLES, description: 'View and manage ABAC (Attribute-Based Access Control) policies per role and resource.', group: 'admin-governance', navVisible: true, path: '/admin/access-policies', title: 'Access Policies' },
  ...
  { allowedRoles: ADMIN_ROLES, description: 'Configure who approves each governed action (project activation, budget change, person release, etc.).', group: 'admin-governance', navVisible: true, path: '/admin/responsibility-matrix', title: 'Responsibility Matrix' },
  { allowedRoles: ADMIN_ROLES, description: 'Manage bill-rate cards and per-(role × grade × skill) hourly rates used by the J2 resolver at assignment-book time.', group: 'admin-config', navVisible: true, path: '/admin/rate-cards', title: 'Rate Cards' },
  { allowedRoles: ADMIN_ROLES, description: 'Author and publish Help Center articles end users see from the in-app `?` button.', group: 'admin-config', navVisible: true, path: '/admin/help', title: 'Help Center' },
  { allowedRoles: ALL_ROLES, description: 'Browse Help Center articles published by your platform admins.', group: 'admin-config', navVisible: true, path: '/help', title: 'Help' },
];
