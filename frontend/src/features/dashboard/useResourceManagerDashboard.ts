...
import {
  ResourceManagerDashboardResponse,
  fetchResourceManagerDashboard,
} from '@/lib/api/dashboard-resource-manager';
import { fetchPersonDirectory } from '@/lib/api/person-directory';


interface ResourceManagerDashboardState {
  asOf: string;
  data: ResourceManagerDashboardResponse | null;
  error: string | null;
  isLoading: boolean;
...
export function useResourceManagerDashboard(
  ...
  const [data, setData] = useState<ResourceManagerDashboardResponse | null>(null);
  ...
  useEffect(() => {
    ...
    if (!personId) {
      setIsLoading(true);
      void fetchPersonDirectory({ page: 1, pageSize: 100, role: 'resource_manager' })
        .then((dir) => {
          if (!active) return;
    ...
    void Promise.all([
      fetchPersonDirectory({ page: 1, pageSize: 100, role: 'resource_manager' }),
      fetchResourceManagerDashboard(personId, asOf),
    ])
      ...
      .catch((reason: unknown) => {
        ...
        setError(
          reason instanceof Error
            ? reason.message
            : 'Failed to load resource manager dashboard.',
        );
      })
      .finally(() => {
        if (active) {
