...
| Phase V2 | DS Redesign Operational Cutover | V2-A..V2-H + V2-X | 🔄 In Progress (2026-05-26): **ds-conformance 39→0** (100% reduction) + ratchet wired (Husky + CI) + DS Tabs atom shipped + **V2-B.4 SparklineDs CLOSED** (legacy file deleted). **~28/132 items done (~21%)**. PRs #342–#367 merged or in flight (25+). Frame/sidebar/leave-bug done; conformance ratchet locked. Atom adoption: Money 2→13, Pct 2→75+, Sparkline → SparklineDs (all migrated, legacy retired). Next: V2-A surface work (Plan tab Gantt, Bench inspector, Approvals one-screen-approval, 3-tab Project Detail consolidation). |
...
- [ ] **V2-B.2** `Pct` adoption — **~75+ callsites** as of 2026-05-26 (was 2). Sweeps shipped: PRs #361 (slice 1), #362 (slice 2 RM dashboard + WorkloadCard), #364 (slice 3 — 8 routes / 11 sites), #365 (Capitalisation + PlannerAutoMatchPreviewModal), #366 (slice 4 — 16 routes / 19 sites), #367 (slice 5 — 9 routes / 14 sites). Continuing target ≥100. — FE
