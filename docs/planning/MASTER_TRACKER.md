...
- [ ] **D-103** — [GAP] Schema-wide actor-audit gap — 0/105 models have `createdById`/`updatedById`. Decide: denormalize on rows (cost M) or formalize "use AuditLog joins" with documented patterns + lint rule. _partial: 28/105 models. Rounds 1-12: F-10.3..F-42 (22), F-44 (ProjectChangeRequest + ProjectActivationApproval — governance), F-46 (HelpArticle + ProjectVendorEngagement — CMS + financial), F-48 (ProjectWorkstream + WorkflowDefinition — project sub-aggregate + workflow definitions). Remaining ~77._ — `data-quality-audit.md` Part 1 finding 1
