...
- [ ] **DM-5-5** Add missing `updatedAt`; add uniform `createdByPersonId` / `updatedByPersonId` on high-audit tables — BE _partial 2026-05-19 / F-29 (round 4): added on `TimesheetWeek` + `ProjectRisk`. Combined with F-10.3 + F-17 + F-26, 8 aggregates now carry the columns. `StaffingRequest` excluded — `requestedByPersonId` is a legacy non-UUID `String` column needing type correction first. Remaining: ~97 lower-audit tables; per-batch as service-layer adoption catches up._
...
- [ ] **D-103** — [GAP] Schema-wide actor-audit gap — 0/105 models have `createdById`/`updatedById`. Decide: denormalize on rows (cost M) or formalize "use AuditLog joins" with documented patterns + lint rule. _partial: 8/105 models carry the columns. Sprint F-10.3 (Project + ProjectAssignment), Sprint F-17 (CaseRecord + PersonReleaseRequest), Sprint F-26 (LeaveRequest + AssignmentApproval), Sprint F-29 (TimesheetWeek + ProjectRisk). Service-layer adoption + full sweep across remaining 97 models continues per-batch._ — `data-quality-audit.md` Part 1 finding 1
