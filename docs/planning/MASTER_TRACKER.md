...
| Phase 19 | Employee Lifecycle Activity Feed & Data Integrity | — | ✅ Complete (10/10; final item 19-10 closed Sprint F-11.9, 2026-05-16) |
...
| Phase 20c | Architecture & Clean Code | — | 🔄 In Progress (8/18 done: 20c-02/04/07/09/12/13/14/17; 20c-11 partial 126/146 sites; 2026-06-11) |
...
| Phase 21 | Feature Uplift (from FEATURE_IMPROVEMENTS.md audit) | — | 🔄 17/20 — Phase 0 ✅ (5/5 + infra), prereqs 2/3 (20c-15 dashboard split open), Sprint A 6/6, Sprint B 3/5 (21-07 + 21-11 obsoleted → Cat-2) (2026-06-11) |
| Phase WO | Assignment Workflow Overhaul (slate, IN_REVIEW, SLA, admin-configurable thresholds) | — | `[-]` **Superseded** by the lean `ProjectPosition` aggregate + position-flows remediation (#658–#687) — see [charter §5](ALIGNMENT-AND-DEVELOPMENT-LOOP.md). WO-4.12's target `StaffingRequestDetailPage` was deleted by V2 PR 17b; WO-6 cutover is overtaken by the lean migration. _(Historical: 48/49 — WO-1..WO-3 ✅, WO-4 14/15, WO-5 ✅.)_ |
...
| Phase V2 | DS Redesign Operational Cutover | V2-A..V2-H + V2-X | 🔄 In Progress (2026-05-26): **ds-conformance 39→0** + ratchet wired (Husky + CI) + DS Tabs atom shipped + **V2-B.4 SparklineDs CLOSED** + **V2-B.6 VarianceBar CLOSED** + **12 V2-A surfaces CLOSED/partial** (V2-A.1 #371 · V2-A.4 #373 · V2-A.5 #370 · V2-A.6 #370 · V2-A.7 #369 · V2-A.8 #372 · V2-A.10 #375 · V2-A.12 partial #377 · V2-A.14 #360 · V2-A.15 partial #391 · V2-A.17 #389 · V2-A.18 accepted) + **V2-F token cleanup COMPLETE** (raw-hex baseline 335→**0** across 25 slices, PRs #395/#396/#401–#413 + #415–#426; `no-raw-hex` now an error-tier hard gate). **52 `[x]` + 8 `[-]` of 164 section checkboxes as of 2026-06-11** — the section is superseded by the V2 source-of-truth plan (`~/.claude/plans/v2-source-of-truth-2026-06-09.md`; 19/20 SoT PRs shipped, remaining: PR 20 `dsRefresh` C0 flip), so most open `[ ]` items here are stale rather than pending. PRs #342–#429 merged. **⚑ 2026-05-29 BA validation** (`docs/planning/v2-plan-validation-2026-05.md`): the done-% is **DS-adoption scope**; the strategic **lean-flow-go-live scope is ≈5–10%** — `ProjectPosition` is built but parallel/shelved (live surfaces still read legacy `ProjectAssignment`/`StaffingRequest`). See the **Strategic Track Map (Bank Go-Live)** at the head of the Phase V2 section: Track 1 (lean go-live, incl. `NEW-LGL-1` read re-point + V2-H.13/H.14) is the real critical path. Next per validation: Track 1 (BE, concurrent-territory — confirm ownership) or Track 4 DS-finish for bank surfaces. **⚑ 2026-05-29 wire-by-wire process assessment** (`docs/planning/v2-bank-process-assessment-2026-05.md`, 12-agent workflow): 50 addressed actions (A-01…H-01) across the 4 bank processes, reconciled drop list, DS-alignment matrix; added NEW-LGL-6…12. Key correction: legacy load ≈34 svc files (not 21). |
...
- [-] **20c-03** Rename/refactor `in-memory-staffing-request.service.ts` — named "in-memory" but uses Prisma directly (40+ calls). Rename to `PrismaStaffingRequestService` and add repository abstraction — BE · Severity: High _obsolete 2026-06-11: the file was deleted with the StaffingRequest aggregate in the lean migration (LEAN-P3-2 drop, `20260609_lean_p3_2_drop_legacy_tables`)._ _F-14.3 deferred 2026-05-17: 40+ Prisma calls + cross-module callers (workforce-planner, dashboards, staffing-desk). Rename touches every consumer + needs a port interface that captures all 40 method signatures. Dedicated PR (cluster B mid-sprint) when next prioritized._
...
- [x] **20c-06** Split `AuthService` (498 lines, 16 methods) into: `AuthenticationService`, `TwoFactorService`, `PasswordManagementService`, `AccountManagementService` — BE · Severity: High _(verified shipped — `src/modules/auth/` now holds AuthService at 209 lines plus extracted `two-factor.service.ts`, `password.service.ts`, `token.service.ts`, `oidc.service.ts`, `sso-admin.service.ts`, each with a spec in `test/unit/auth/`)_
...
- [x] **20c-10** Replace `any` types in 15+ Prisma repository Gateway interfaces — all use `args: any` and `Promise<any>`. Create properly typed Gateway generics — BE · Severity: High _(verified shipped — zero live `args: any` / `Promise<any>` left in `src/`; repos carry "20c-10 — typed Prisma delegate slice" markers, e.g. `src/modules/integrations/m365/infrastructure/repositories/prisma/prisma-m365-directory-reconciliation-record.repository.ts`, `src/modules/organization/infrastructure/repositories/prisma/prisma-org-unit.repository.ts`)_
...
- [-] **20c-18** Refactor boolean parameters in `CreateProjectAssignmentService` — `allowOverlapOverride`, `draft`, `projectValidated`, `personValidated` flags violate SRP. Use builder pattern or separate services — BE · Severity: Medium _obsolete 2026-06-11: `CreateProjectAssignmentService` was deleted with the ProjectAssignment aggregate (LEAN-P3-2 drop); lean positions use `TransitionProjectPositionFillService`._ _F-14.6 deferred 2026-05-17: 4 flags exist on the Command shape (already a named-args pattern, not positional). The real SRP fix is to extract the 3 validation steps (`personValidated`, `projectValidated`, overlap-check) into composable steps the orchestrator can opt out of — not a builder. Defer until the underlying validation seam is unified._
...
- [ ] **DM-2.5-8** Migrate controllers to publicId top-down per `docs/planning/dm2.5-controller-migration-template.md`. One sub-task per aggregate. _audit-2026-05-15: partial: 2/10 aggregates done (Skill A+B, StaffingRequest A+B); 8 aggregates remaining behind code-heavy controller rollout_
   ...
   - [-] **DM-2.5-8-staffing** _obsolete 2026-06-11: StaffingRequest aggregate + tables dropped in the lean migration (LEAN-P3-2, `20260609_lean_p3_2_drop_legacy_tables`); no Sub-C to ship._ Sub-A + B landed for StaffingRequest. **Sub-A** (2026-04-18): `StaffingRequest` interface + `PrismaRecord` gain publicId; `toResponse()` emits publicId as `id` field with uuid fallback; `findRecordByIdOrPublicId` transitional helper. **Sub-B** (2026-04-19): all 7 controller endpoints (`GET /:id`, `PATCH /:id`, `POST /:id/submit`, `/review`, `/release`, `/fulfil`, `/cancel`) use `ParsePublicIdOrUuid(AggregateType.StaffingRequest)`; service layer adds `resolveInternalId()` that routes every mutation through the helper first. Backend `tsc --project tsconfig.build.json --noEmit` clean. Sub-C (drop uuid + flip to strict `ParsePublicId`) is blocked on DM-2.5-9 frontend route flip. — BE · _(sub-A+B done 2026-04-19)_
   ...
   - [-] **DM-2.5-8-assignment** Sub-phases A/B/C for ProjectAssignment — BE · _obsolete 2026-06-11: ProjectAssignment aggregate + tables dropped in the lean migration (LEAN-P3-2); successor `ProjectPosition` already carries `publicId`._
...
- [-] **WO-4.12** `StaffingRequestDetailPage` redesign with `<DetailLayout>` + `<WorkflowStages>` strip + stage-specific action card — FE _obsolete 2026-06-11: page deleted with the StaffingRequest aggregate (LEAN-P3-2); successor surface is `ProjectPositionDetailPage`._
...
- [-] **D-95** — [DERIVE] Replace `StaffingRequest.headcountFulfilled` cached counter with a derived count — adjacent to D-11 status drift, candidate to fold into S-13 scope. — register row #5, §2 _obsolete 2026-06-11: `StaffingRequest` table + column dropped in the lean migration (LEAN-P3-2)._
...
- [-] **D-99** — [REFACTOR] `WorkforcePlannerService.applyPlan` calls `prisma.projectAssignment.create()` directly — bypasses `CreateProjectAssignmentService` (no audit log, no transaction wrap). _obsolete 2026-06-11: ProjectAssignment + `CreateProjectAssignmentService` dropped in the lean migration; `applyPlan` now creates `ProjectPosition` rows inside a `$transaction` (`workforce-planner.service.ts:1580`)._ _audit-2026-05-17: F-15.9 deferred 2026-05-17: `WorkforcePlannerService.applyPlan` calls `prisma.projectAssignment.create()` for bulk dispatches. Routing through `CreateProjectAssignmentService.execute()` would add validation/conflict checks the planner has already done — needs either a `bypassValidation` flag on the service or a separate `BulkCreateForPlannerService`. Service-layer rework; dedicated PR with integration test coverage._ — register row #12, §5b
...
- [-] **BUG-SR-1 — Cancelled staffing request renders as Open; slate "Pick" CTA throws 409.** _obsolete 2026-06-11: every implicated surface (StaffingRequest aggregate, `derive-staffing-request-status.service.ts`, `in-memory-staffing-request.service.ts`, `StaffingRequestDetailPage.tsx`) was deleted in the lean migration (LEAN-P3-2); ProjectPosition lifecycle has no equivalent state._ Browser repro 2026-05-20 on `https://deliverit-test.agentic.uz/staffing-requests/c9085c62-…`: SR has `status='CANCELLED'` + `cancelledAt` set (via `POST /staffing-requests/:id/cancel`), but the detail page shows derivedStatus **Open**, workflow timeline pinned at the **Open** step, and the proposal slate still renders an active "Pick selected candidate" CTA. Clicking it surfaces the backend's correct refusal: _"Staffing request stf_… must be OPEN or IN_REVIEW to pick a candidate (current: CANCELLED)."_ The UI invites an action the backend rejects — the rendered state is a lie.
...
### Phase V2 — Strategic Track Map (Bank Go-Live) — added 2026-05-29

**Why this exists:** the 2026-05-29 product/BA validation (`docs/planning/v2-plan-validation-2026-05.md`) found the sub-phases below optimize **DS-adoption** (~41% done) while the **lean-flow go-live** that delivers the bank's actual value (supply×demand / staffing / resource planning on `ProjectPosition`) is **≈5–10%** — the `ProjectPosition` aggregate is built but **parallel/shelved**; every live surface still reads the legacy `ProjectAssignment`/`StaffingRequest` model. This map re-orders existing items by strategic value **without** moving them (the sub-phase lists below are unchanged). `NEW-LGL-*` are gaps the validation surfaced that have no existing item.

- **Track 1 — Lean Go-Live (CRITICAL PATH; nothing else moves the needle until this lands):** `V2-H.10` backfill → **`NEW-LGL-1`** re-point live reads (staffing-desk / supply+demand-profile / dashboards) from legacy models to `ProjectPosition.activeFill` → `V2-H.13` migrate 21 callsites → `V2-H.14` drop 10 legacy models (forward-only, docs' #1 risk) → `V2-H.15` deprecated-import ratchet → `V2-H.11`/`V2-H.12` positions + bench skeleton pages. _BE-heavy; overlaps concurrent-agent staffing/workstream territory — confirm ownership before starting._
- **Track 2 — Bank Day-1 config:** **`NEW-LGL-2`** SSO admin UI (D-155) · **`NEW-LGL-3`** custom-role admin UI (D-159 — Squad/Tribe Lead, IT Service Owner) · `V2-H.3` base-currency admin tab · `V2-H.19`/`V2-H.20` currency + fiscal flag flips · locale/fiscal **consumption** (settings exist, unused — D-160a/161/163/165) · **`NEW-LGL-4`** Create-Project-Wizard accelerator (pre-fill positions/milestones from bank template).
- **Track 3 — Resource-planning depth:** `V2-C.11`/`V2-C.12`/`V2-C.13` Distribution Studio swimlane + heatmap + bench sidebar · `V2-H.8` capacity reserves approved leave · `V2-A.3` Plan-tab Gantt (once workstream BE lands) · `V2-C.1`/`V2-C.2`/`V2-C.3` editable-Timeline props (build **only** when the Distribution Studio consumer is in the same sprint — avoid speculative atom props).
- **Track 4 — DS finish for bank-critical surfaces only:** **`NEW-LGL-5`** Law-7 one-screen unified approvals (position-proposal + leave + activation + skill-review, inline approve/reject) · the ~4 canvas-wiring gaps (JQL bar, Gantt render, Leave drawer, Profile-360) · `V2-B.13` Icon set → `V2-A.11a` MUI removal. **Drop** the V2-B callsite-count targets (B.1 Money ≥50, B.3 Donut ≥6, B.5 MiniBars ≥10) — adoption counters, not user value; re-scope to "DS atoms on bank-critical surfaces."
- **Track 5 — Cutover:** `V2-G.1` visual-regression → `V2-G.3` soak → `V2-G.4` `dsRefresh` prod flip → `V2-G.6`–`V2-G.16` C1 legacy cleanup.

**NEW-LGL gap items** (surfaced by the validation; not yet scheduled):
- [x] **NEW-LGL-1** Re-point live reads to `ProjectPosition.activeFill` — staffing-desk, supply-profile, demand-profile, workforce-planner, dashboards. The bridge from "model built" to "lean flows live." — BE _(verified shipped — staffing-desk/supply/demand/planner/dashboard services all query `prisma.projectPosition` (`src/modules/staffing-desk/application/staffing-desk.service.ts`, `supply-profile.service.ts`, `demand-profile.service.ts`); legacy tables dropped in `20260609_lean_p3_2_drop_legacy_tables`)_
- [x] **NEW-LGL-2** SSO admin configuration UI (D-155) — bank Day-1 go-live blocker per `bank-it-deployment-runbook.md`. — FE+BE _(verified shipped — `frontend/src/routes/admin/SsoAdminPage.tsx` + `.test.tsx` at `/admin/integrations/sso`; backend `src/modules/auth/sso-admin.controller.ts` + `test/unit/auth/sso-admin.controller.spec.ts`)_
- [x] **NEW-LGL-3** Custom-role admin UI (D-159) — tenant adds bank shapes (Squad/Tribe Lead, IT Service Owner). — FE+BE _(verified shipped — `frontend/src/routes/admin/CustomRoleAdminPage.tsx` + `.test.tsx` at `/admin/governance/roles`; backend `src/modules/identity-access/presentation/custom-role.controller.ts` + `test/unit/identity-access/custom-role.controller.spec.ts`)_
- [ ] **NEW-LGL-4** Create-Project-Wizard accelerator — pre-fill positions/milestones from a bank template (the real fast-onboarding lever). — FE+BE
- [x] **NEW-LGL-5** Law-7 one-screen unified approvals — position-proposal + leave + activation + skill-review with inline approve/reject. — FE+BE _(verified shipped — `frontend/src/routes/approvals/ApprovalsPage.tsx` lists all four sources; `frontend/src/components/approvals/ApprovalInspector.tsx` provides inline approve/reject; backend `src/modules/dashboard/application/unified-approval-queue.service.ts` + `test/unit/dashboard/unified-approval-{queue,decision}.spec.ts`)_
- [x] **NEW-LGL-6** Extract `SuggestFillsService` (`suggestForPosition`+`suggestForBatch`) off `ProjectPosition` — matching logic currently embedded in `workforce-planner.autoMatch` over legacy models; backs both planner + single-position Find-Candidates. — BE _(action A-02, Track 3)_ _(verified shipped — `src/modules/project-positions/application/suggest-fills.service.ts` + `test/unit/project-positions/suggest-fills.spec.ts`)_
- [x] **NEW-LGL-7** Build `ProjectPositionDetailPage` (`/projects/:projectId/positions/:positionId`) + `GET /project-positions/:id/candidates` + wire the dark `POST /project-positions/:id/transition` into a reachable flow. V2-H.11 is only a skeleton list; also kills the dead `/positions/:id` cmdk href. — FE+BE _(actions B-01/B-02/B-03, Track 1)_ _(verified shipped — `frontend/src/routes/projects/ProjectPositionDetailPage.tsx` + `.test.tsx`; `/positions/:id` route wired in `router.tsx`; `GET :id/candidates` + transition endpoints in `src/modules/project-positions/presentation/project-positions.controller.ts`)_
- [x] **NEW-LGL-8** Lean `RELEASED` transition replacing dual-approval offboarding — drop-list drops `PersonReleaseRequest`/`PersonReleaseApproval` but nothing builds the replacement. — BE _(action B-06, Track 1)_ _(verified shipped — `RELEASED` is a terminal state reachable from any state with a reason in `src/modules/project-positions/domain/value-objects/position-fill-status.ts` + `test/project-positions/position-fill-status.spec.ts`; `PersonReleaseRequest`/`PersonReleaseApproval` models gone from `prisma/schema.prisma`)_
- [ ] **NEW-LGL-9** Build `GET /me/overview` + `GET /me/memberships` (ProjectPosition-scoped) — the lean self-service payloads; `/me` tabs currently improvise via legacy aggregators. — BE _(actions C-02/C-03, Track 1)_
- [x] **NEW-LGL-10** Repoint role-dashboard query services + `person-profile` + 3 time-domain reads off InMemory/legacy onto `ProjectPosition`. Verifier found the real legacy load ≈ **34 service files** (V2-H.13's "21" is understated, incl. dashboards/profile/time not enumerated). — BE _(actions A-04/A-05/A-06, Track 5)_ _(verified shipped — all dashboard query services + `person-profile.service.ts` read `prisma.projectPosition`; zero live `prisma.projectAssignment`/`prisma.staffingRequest` calls remain in `src/`; legacy tables dropped in `20260609_lean_p3_2_drop_legacy_tables` which records the SoT PR 14/15 repoint)_
- [x] **NEW-LGL-11** `GET /leave-requests/preview` (working-days excl. weekend+PublicHoliday, conflicts, balance-after) + `POST /leave-requests/:id/cancel` (self-cancel PENDING). Server-authoritative; LeaveTab drops client-side math. — BE _(actions C-04/C-05, Track 3; overlaps V2-H.9 calc)_ _(verified shipped — PR 657 / LEAN-P4-missing-11: `@Get('preview')` + `@Post(':id/cancel')` in `src/modules/leave-requests/presentation/leave-requests.controller.ts`, `leave-impact-preview.service.ts` + `test/unit/leave-requests/leave-impact-preview.spec.ts`)_
- [x] **NEW-LGL-12** Emit `leave.approved/rejected/cancelled` outbox + `NotificationEventTranslator` (close **20b-10** — the only OPEN leave loop after 20c-05 closed). — BE _(action C-06, Track 1)_ _(verified shipped — `leave.approved` / `leave.rejected` / `leave.cancelled_by_employee` dual-dispatch through the outbox in `src/modules/notifications/application/notification-event-translator.service.ts` (OUTBOX_HANDLERS registered), wired from `leave-requests.service.ts`; `test/unit/leave-requests/leave-notifications.spec.ts`)_
- [ ] **NEW-LGL-13** `LeavePolicy` model + nightly `LeaveBalanceDriftSweep` — balance becomes a policy-driven derived projection, not a free-Decimal source of truth. — BE _(action C-07, Track 3)_
- [ ] **NEW-LGL-14** `StaffingDeskTab` Prisma model + JQL end-to-end wiring (`lib/api/jql.ts`, mount `JqlQueryBar`, serialize `?tab=`/`?jql=`). V2-C.4–C.9 cover atoms/half-BE but not the persisted-tab model or FE wiring. — FE+BE _(action B-09, Track 3)_
- [ ] **NEW-LGL-15** Clean `cmdk-search` legacy branch + opaque-id hrefs + fix the dead `/positions/:id` link — the one in-area legacy reader; must precede contract migration. — BE _(action A-07, Track 5)_
- [ ] **NEW-LGL-16** Collapse duplicate integrations endpoints + merge `/admin/integrations` + `/registry` into one DS tab (registry table + inspector drawer). — FE+BE _(actions D-02/D-03/D-04, Track 4)_
- [x] **NEW-LGL-17** Subject-employee `AllowSelfScope` read on Cases — onboarding subject sees own case status from `/me`. — BE _(action C-11, Track 3)_ _(verified shipped — W2-01: `GET /me/cases` in `src/modules/case-management/presentation/me-cases.controller.ts` returns only subject/participant cases for the caller; `test/unit/case-management/me-cases-controller.spec.ts`)_
- [ ] **NEW-LGL-18** Overtime-policy CRUD disposition — wire `/admin/time-policies` OR drop for PlatformSettings (4 built-unwired/no-FE endpoints need a build-or-drop call). — FE+BE _(action F-01, Track 3)_
- [ ] **NEW-LGL-19** Flag-gate `/help` route to the existing `flag.helpCenter.enabled` (flags exist; route unwired to them). — FE _(action E-06, Track 2)_
- [ ] **NEW-LGL-20** Retire orphaned `TimesheetPage`/`TimesheetApprovalPage` + dead `/my-time/gaps`, `team-builder`, `fulfil` — dead-surface cleanup not in the V2-G enumeration. — FE+BE _(actions G-01/G-03/G-04, Track 5)_
- [ ] **NEW-LGL-21** Move Case SLA config off `InMemoryCaseSlaService` to `PlatformSetting` (operational persistence gap). — BE _(action C-12, Track 2)_

**📋 Detailed wire-by-wire action plan:** `docs/planning/v2-bank-process-assessment-2026-05.md` — the 2026-05-29 12-agent (Foundation→Assess→Verify→Synthesize) assessment of the 4 bank processes (supply-demand, time-recording, integrations, role-self-service). Contains **50 addressed actions (A-01…H-01)** each with kind/effort/track/DS-atoms/deps/acceptance, a reconciled DROP list (76 drop/defer entries triaged), and a DS-alignment matrix (surface→crafted page→atoms→pattern gap). Verifier corrections folded in — notably: legacy load ≈ **34 service files** (not 21); `resource-pools` is **already Prisma-backed** (no persistence work); `cmdk-search` emits a **dead `/positions/:id`** link to fix; leave-balance write-path **already wired** (20c-05 closed, only 20b-10 remains). The action IDs map to existing tracker items + the NEW-LGL set above (see §5 of the doc).
...
- [x] **V2-A.9** Staffing Desk — build the canvas's killer feature: editable swimlane planner where positions drag across people. `DistributionStudio.tsx:50-302` currently ships a scenarios list with strategy chips (admitted by the file's own comment at `:44-49`). See V2-C for sub-tasks. — FE + BE _(verified shipped — SoT PR 7 collapsed the scenarios list into the canonical planner: `DistributionStudio.tsx` now mounts `frontend/src/components/staffing-desk/WorkforcePlanner.tsx` with drag-across-people cells, bench drag-in, heat layers, and simulation/apply; `DistributionStudio.test.tsx` asserts the bench-sidebar+grid surface)_
...
- [-] **V2-A.15** Create Project Wizard — replace `basics → engagement → review` step shape with canvas's `Identity → Positions+Milestones → Activation` per `DS/page-extras.jsx`. _2026-05-26 partial close (PR #391): step indicator + page subtitle + SectionCard title now use canvas labels (`Identity / Setup / Activation`). Post-create card gained a "Next: add positions and milestones" hint + an "Open Plan tab" primary CTA deep-linking to `/projects/<id>?tab=plan`. Full inline Positions+Milestones step is V2-A.15-followup — needs a Prisma transaction to create project + positions + milestones atomically._ — FE
- [x] **V2-A.16** Time Management — migrate `LeaveDecisionDrawer.tsx` raw `<button>`/`<table>` to DS atoms. _(already done — footer migrated in PR #346 (V2-E.4), conflicts table in PR #354 (V2-E.14); LeaveDecisionDrawer now has zero raw button/table.)_ — FE
- [x] **V2-A.17** Time Management — surface `LeaveDecisionDrawer` as `/approvals?source=leave` filtered view per reconciliation §5 decision (folds time-management into Approvals). _shipped 2026-05-26 (PR #389): `ApprovalInspector` gained a "Leave detail" block (between Variance and Submitter Rationale) that renders 4 opportunistic tiles when `item.source === 'leave'` and `item.meta` carries known leave keys — Type / Dates (start → end) / Business days / Balance after (danger color when negative). Operator decides leave requests inline without the `/time-management` round-trip. Timesheet folding is V2-A.17-followup — the unified queue's `ApprovalQueueSource` union doesn't yet expose a `timesheet` value, so it needs BE schema work._ — FE
...
- [x] **V2-B.9** `BalanceMeter` adoption — _(already complete, verified 2026-05-29): the only leave-balance surfaces in the app — `LeaveTab.tsx` + `LeaveDecisionDrawer.tsx` — both use `<BalanceMeter>`. A codebase sweep found zero other entitlement/used/remaining renders. Manager dashboards/capacity rollups show utilization %, not leave-entitlement balances, so BalanceMeter is not the right atom there. Coverage = 100% of applicable surfaces._ — FE
- [-] **V2-B.10** `Avatar` adoption — **~28 importing files** as of 2026-05-26 (was 6, original target ≥15 hit). Sweeps shipped: PR #381 slice 1 (StaffingDeskTable, BenchDashboard, TimeManagementPage ×2, TimesheetApprovalPage, RmDetailTables ×2, TeamTab — 6 routes / 7 callsites), PR #383 slice 2 (TeamDashboardPage ×2, ResourcePoolDetailPage, AnomalyPanel, ApprovalQueuePage, BulkAssignmentForm — 5 routes / 6 callsites), PR #386 slice 3 (TeamVendorsTab ×2, ProjectDashboardPage, AssignmentDetailsPlaceholderPage, TeamDashboardPage missed-by-slice-2, PlannedVsActualPage, DataQualityTab — 6 routes / 7 callsites), PR #388 slice 4 (TimeManagementPage overtime tab, BenchDashboard rolloff row, LifecycleTab ×2 — 3 routes / 4 callsites). Marked [-] for long-tail adoption — remaining person-render surfaces will be backfilled as touched. — FE
- [-] **V2-B.11** `Timeline` (DS) adoption — **~5 importing files** as of 2026-05-26 (was 4). Sweeps shipped: PR #393 (`PersonProfilePanel` — Timeline above assignments list with `showToday` + `showMonthLabels` + `size="sm"`; segments map to project deep-links via `href`). `StaffingDeskTable` row timelines were already covered via the existing `WorkloadTimeline` wrapper (which mounts the DS `Timeline` atom). `Pulse activity` and `Profile activity` items are vertical event feeds, not horizontal date-bound segments — semantically distinct from Timeline; those stay as bespoke list renders. — FE
...
- [x] **V2-C.4** JQL field registry — typed schema of filterable fields per amendment §1.4. — FE + BE _(verified shipped — per-scope field whitelist + typed AST in `src/modules/staffing-desk/application/jql/jql-types.ts`; whitelist rejection covered by `test/unit/staffing-desk/jql-parser.spec.ts`)_
- [x] **V2-C.5** JQL parser + tokenizer — promote existing prototype at `frontend/src/features/staffing-desk/jql-tokenizer.ts` + `jql-parser.ts` to first-class with inline error highlighting. — FE _(verified shipped — FE-#268: backend `jql/jql-parser.ts` + FE tokenizer/parser; `JqlQueryBar.tsx` renders inline error state; `test/unit/staffing-desk/jql-parser.spec.ts`)_
- [x] **V2-C.6** JQL execution endpoint `POST /api/staffing/jql/execute` — server-side translation to Prisma; RBAC-scoped. — BE _(verified shipped — `@Post('execute')` in `src/modules/staffing-desk/presentation/jql.controller.ts` with `@RequireRoles(...STAFFING_ROLES)`; translation in `jql-to-prisma.ts` covered by `jql-parser.spec.ts`)_
- [x] **V2-C.7** JQL parse-only endpoint `POST /api/staffing/jql/parse` — for autocomplete + validation. — BE _(verified shipped — `@Post('parse')` in `src/modules/staffing-desk/presentation/jql.controller.ts` returning AST + validation errors)_
...
- [x] **V2-C.10** Saved-tab strip UI — render 6 tabs (mine + public examples) above the JQL bar with `Public`/`Mine` scope chips per `DS/page-staffing-desk.jsx:5-44`. — FE _(verified shipped — SoT PR 6: `frontend/src/components/staffing-desk/StaffingDeskTabStrip.tsx` (6 default tabs, private/public scope) mounted above the JqlBar in `StaffingDeskPage.tsx` with `?tab=` URL persistence; tabs persist in localStorage rather than the V2-C.8 server model)_
- [x] **V2-C.11** Distribution Studio swimlane view — replace current scenarios-list with editable Timeline + swimlanes by person + position dragging. — FE _(verified shipped — SoT PR 7: scenarios-list replaced by `WorkforcePlanner` grouped-lane grid with cell/bench dragging; scenario CRUD moved to `PlannerScenariosMenu`; `DistributionStudio.test.tsx`)_
- [x] **V2-C.12** Distribution Studio heatmap toggle — coverage / cost / match / risk layers per amendment. — FE _(verified shipped — `HeatLayer = 'coverage' | 'cost' | 'match' | 'risk'` toggle in `frontend/src/components/staffing-desk/WorkforcePlanner.tsx`)_
- [x] **V2-C.13** Distribution Studio bench sidebar — show idle people inline alongside the planner. — FE _(verified shipped — `frontend/src/components/staffing-desk/PlannerBenchSidebar.tsx` mounted in `WorkforcePlanner`; asserted by `DistributionStudio.test.tsx`)_
...
- [x] **V2-D.2** `window.prompt` removal — `DistributionStudio.tsx:88` done via V2-C.14 (PR #428); the two remaining `window.prompt` calls in `PlannerScenariosMenu.tsx` (save + fork) replaced with one parameterized DS `FormModal` in PR #429 (+ new test file, 2 tests). No `window.prompt`/`window.alert` left in app code. — FE
...
Each row = one file's hex-color migration to design tokens. Target = baseline reaches 0 then promote the rule to `error`-tier. ✅ **COMPLETE 2026-05-28 — baseline 335 → 0 across 25 slices (PRs #395/#396/#401–#413 + #415–#426).**

- [x] **V2-F.1** `components/org/OrgChartNode.tsx` — 50 raw hex → status / chart tokens. **Largest concentration.** _(slice 14, PR #415; HTML-string node renderers — `var()`/`color-mix()` resolve in d3 foreignObjects.)_ — FE
- [x] **V2-F.2** `routes/workload/WorkloadPlanningPage.tsx` — 33 raw hex (incl. inline `AVATAR_COLORS` palette). _(slice 16, PR #417; `AVATAR_COLORS` → `--color-chart-1..7`; heat ramp → `color-mix(<status> N%, --color-surface)`; shared `workload-helpers.ts` migrated alongside.)_ — FE
- [x] **V2-F.3** `components/staffing-desk/WorkforcePlanner.tsx` — 17 raw hex. _(slice 17, PR #418; dynamic-alpha rgba → `color-mix(... ${expr*100}%, transparent)`.)_ — FE
- [x] **V2-F.4** `components/org/DepartmentSidebarDrawer.tsx` — 12 raw hex. _(slice 15, PR #416.)_ — FE
- [x] **V2-F.5** `components/org/PersonSidebarDrawer.tsx` — 12 raw hex. _(slice 15, PR #416; +10 undefined-token refs fixed to canonical tokens.)_ — FE
- [x] **V2-F.6** Recharts `fill="#hex"` props — `routes/reports/CapitalisationPage.tsx` (slice 24 #425), `routes/dashboard/DeliveryManagerDashboardPage.tsx` (slice 24 #425), and the whole `components/charts/` directory (slices 18/19, PRs #419/#420). `UtilizationPage.tsx` carried no baselined hex. _(`var()` resolves in recharts/SVG presentation attrs — DS-atom precedent: Donut/MiniBars.)_ — FE
- [x] **V2-F.7** Long-tail — swept file-by-file, ratcheting baseline to 0. _2026-05-28 DONE: **335 → 0** across 25 slices (PRs #395/#396/#401–#404/#406–#413, then #415–#426). Mappings: multi-color recharts palettes → `--color-chart-1..8`; status maps → `--color-status-*`; alpha fills → `color-mix(<token> N%, transparent)`; grey/white text → `--color-text`/`-muted`/`-subtle`/`--color-text-inverse`; track/grid bg → `--color-border`; modal backdrops → `--color-overlay`; box-shadows → `--shadow-dropdown/-modal/-card`; undefined-token fallbacks (success/error/warning/primary/text-tertiary/on-status) → canonical existing tokens. ImpersonationBanner resolved via theme-invariant `--color-impersonation-bg/-fg` in `sharedTokens` (slice 25, PR #426). Baseline edits done line-based / via Node `JSON.stringify` (never python json.dump — escapes emoji/em-dash)._ — FE
- [x] **V2-F.8** `no-raw-hex` rule is now an `error`-tier hard gate. _Achieved by the empty baseline: `scripts/check-design-tokens.cjs` already `process.exit(1)`s on any non-baselined raw color, so with `design-token-baseline.json` = `[]` every raw hex/rgba outside `design-tokens.ts` (and test dirs) fails CI. No script change needed; the baseline mechanism is retained for unavoidable future exceptions (e.g. chart-lib requirements) via `--write-baseline`._ — FE
...
- [-] **V2-H.1** Amend S4-6 acceptance criteria — require inspector drawer with inline approve/reject for each of the 5 approval sources, NOT a deep-link list. Edit `lean-simplification-initiative.md` §3.5 Sprint 4 row S4-6. — docs _obsolete 2026-06-11: moot — S4-6 already shipped beyond the amended criteria (`ApprovalInspector.tsx` inspector drawer with inline approve/reject for every source); amending the historical plan doc serves no purpose._
- [-] **V2-H.2** Add story `S5-3a` to spine — `SkillReviewPrompt` + `OnboardingTask` models + producers. Without these, `PeopleHrQueuePage` aggregates nothing for skills/onboarding tabs. — BE _obsolete 2026-06-11: superseded — skill reviews ship via `PersonSkill.selfEndorsed` (LEAN-P4D-4) and onboarding via `CaseRecord`, both already feeding `unified-approval-queue.service.ts`; `PeopleHrQueuePage` itself was superseded by the unified `/approvals` queue (see V2-H.27)._
...
- [x] **V2-H.10** S2 finishing — backfill script `prisma/scripts/backfill-project-positions.ts` (idempotent against `it-company` seed). — BE _(verified shipped — landed as idempotent migrations instead of a standalone script: `20260602_lean_p0_5_reconciliation_backfill` + `20260606_lean_p3_1_data_fixup` reconcile ProjectPosition rows/legacy IDs; legacy sources since dropped, so no further backfill is possible or needed)_
- [x] **V2-H.11** S2 finishing — `/projects/:id/positions` skeleton page reading new endpoints. — FE _(verified shipped — `frontend/src/routes/projects/PositionsListPage.tsx` wired at `projects/:projectId/positions` in `router.tsx`; detail flow covered by `ProjectPositionDetailPage.test.tsx`)_
- [x] **V2-H.12** S2 finishing — `/people/bench` skeleton page reading new endpoints (some overlap with V2-A.7). — FE _(verified shipped — `frontend/src/routes/people/BenchPage.tsx` at `/people/bench` reading `lib/api/people-bench.ts`; backend `list-enriched-bench.service.ts` + `test/unit/project-positions/list-enriched-bench.spec.ts`)_
...
- [x] **V2-H.15** S5 contract phase — `scripts/check-deprecated-assignment-import.cjs` ratchet to 0. — FE + BE _Closed 2026-06-10 via SoT PR 17b. Empirically validated: `grep -rE 'from.*api/assignments|from.*api/staffing-requests' frontend/src` returns zero (V2-done criterion 8). The standalone ratchet script is moot now that the legacy clients themselves are deleted (callers cannot import what doesn't exist); the import drift is caught at the TypeScript compile + lean-migration test layer instead._
- [x] **V2-H.16** S5 contract phase — delete `frontend/src/components/ds-legacy/` (if exists) + legacy CSS classes from `global.css`. — FE _Closed 2026-06-10. `frontend/src/components/ds-legacy/` never existed (verified via `test -d`); SoT PR 17a + 17b deleted the 9 actual legacy components (`AssignmentsTable`, `BulkAssignmentResults`, `AssignmentTable`, `useAssignmentDetails`, `workflow-progression`, `ProposalBuilderDrawer`, `ProposalReviewPanel`, `StaffingRequestDrawer`, `StaffingRequestForm` + validation). Legacy CSS sweep happens implicitly during the V2-F token cleanup cycle (already at raw-hex baseline 0)._
...
- [x] **V2-H.18** S4 wiring — `EvmComputationService` output into `ProjectBudget` rollup + `ProjectRadiator` Budget quadrant. — BE _(verified shipped — `evm-computation.service.ts:192` writes EV/AC/EAC onto `ProjectBudget`; Radiator Budget collectors (CPI, spendRate, forecastAccuracy, capexCompliance) read those columns in `src/modules/project-registry/application/radiator-signal-collector.service.ts`; `test/unit/financial-governance/evm-computation.spec.ts`)_
...
- [x] **V2-H.21** S4 wiring — install `BudgetApproval` auto-trigger listener on `ProjectBudget` mutations > X% variance (configurable per project via PlatformSetting). Service exists but isn't wired. — BE _(verified shipped — `budget-approval-auto-trigger.service.ts` invoked from `financial.service.ts:278` on budget mutations, threshold via PlatformSettings; `test/unit/financial-governance/budget-approval-auto-trigger.spec.ts`)_
- [ ] **V2-H.22** S4 wiring — operator runbook `docs/runbooks/budget-eom-close.md`. — docs
- [x] **V2-H.23** S3 finishing — `ProjectPulseQueryService` endpoint `GET /api/projects/:id/pulse`. (Some shipping under D1.) Verify aggregation matches canvas. — BE _(verified shipped — `@Get('projects/:id/pulse')` in `src/modules/project-registry/presentation/project-pulse.controller.ts` backed by `project-pulse-query.service.ts`; `test/unit/project-registry/project-pulse-query.spec.ts`)_
- [x] **V2-H.24** S3 finishing — Activation approval queue row visible in unified `/admin/approvals`. — BE + FE _(verified shipped — `activation` source in `src/modules/dashboard/application/unified-approval-queue.service.ts` (ProjectActivationApproval rows) + "Activations" chip in `ApprovalsPage.tsx`; `test/unit/dashboard/unified-approval-queue.spec.ts`)_
...
- [-] **V2-H.27** S5 finishing — `PeopleHrQueuePage` aggregator: leave + onboarding tasks + offboarding tasks + skill reviews. — FE _obsolete 2026-06-11: superseded by the unified `/approvals` queue — `ApprovalsPage.tsx` + `unified-approval-queue.service.ts` aggregate leave, case (onboarding/offboarding), and skill-review rows; no separate HR queue page planned._
- [x] **V2-H.28** Org-unit effective-dated move history (RM JTBD #3). Verify `OrgUnitMembership` already carries `validFrom`/`validTo`; if not, add as schema delta. — BE _(verified — `PersonOrgMembership` in `prisma/schema.prisma` carries `validFrom`/`validTo` with `@@unique([personId, orgUnitId, validFrom])` + temporal indexes; no schema delta needed)_
...
- [-] **V2-X.5** Add `@deprecated` JSDoc to `frontend/src/components/charts/Sparkline.tsx` + grep for `JustifyCalendar` other legacy aliases needing the same. — FE _obsolete 2026-06-11: the legacy `charts/Sparkline.tsx` file has been deleted (only `ds/SparklineDs.tsx` remains) and `JustifyCalendar` has zero hits in `frontend/src` — nothing left to deprecate._
