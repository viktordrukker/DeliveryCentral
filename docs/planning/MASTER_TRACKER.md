...
- [ ] **20c-11** Fix dangerous `as unknown as` type assertions — 12+ instances in cases.controller, hr-dashboard-query, InteractiveOrgChart, SettingsPage. Create proper TypeScript interfaces — BE/FE · Severity: Medium _partial: 40/121 sites closed. F-9.3 / PR #77 closed 7 Decimal-cast sites. F-31 closed 12 sites in `help.service.ts` (GetPayload pattern). F-33 closed 10 sites in `rate-card-admin.service.ts` (Prisma client cast). F-35 closed 11 payload casts in `notification-event-translator.service.ts` by widening `sendEmail(payload: object)` and centralising the `Record<string, unknown>` conversion inside the method (1 dynamic-method-lookup cast at line 153 kept as the legitimate pattern). ~90 sites remain across in-memory-staffing-request (10), decide-budget-change (6), staffing-proposal-slate (5), and other services._
...
- [ ] **DM-5-5** Add missing `updatedAt`; add uniform `createdByPersonId` / `updatedByPersonId` on high-audit tables — BE _partial 2026-05-19 / F-34 (round 7): added on `ResourcePool` + `Position` (full org-structure coverage with F-32 OrgUnit). 14 aggregates now carry the columns. Remaining: ~91 lower-audit tables._
...
- [ ] **D-103** — [GAP] Schema-wide actor-audit gap — 0/105 models have `createdById`/`updatedById`. Decide: denormalize on rows (cost M) or formalize "use AuditLog joins" with documented patterns + lint rule. _partial: 14/105 models carry the columns. Rounds: F-10.3 (Project + ProjectAssignment), F-17 (CaseRecord + PersonReleaseRequest), F-26 (LeaveRequest + AssignmentApproval), F-29 (TimesheetWeek + ProjectRisk), F-30 (ProjectMilestone + BudgetApproval), F-32 (OrgUnit + StaffingRequestProposalSlate), F-34 (ResourcePool + Position). Service-layer adoption + full sweep across remaining 91 models continues per-batch._ — `data-quality-audit.md` Part 1 finding 1
