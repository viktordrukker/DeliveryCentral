...
- [ ] **20c-11** Fix dangerous `as unknown as` type assertions — 12+ instances in cases.controller, hr-dashboard-query, InteractiveOrgChart, SettingsPage. Create proper TypeScript interfaces — BE/FE · Severity: Medium _partial: 29/121 sites closed. F-9.3 / PR #77 closed 7 Decimal-cast sites. F-31 closed 12 sites in `help.service.ts`. F-33 closed 10 sites in `rate-card-admin.service.ts` (Prisma client cast pattern via `Prisma.validator<>` + `GetPayload`; 13/13 unit tests pass). ~99 sites remain across notification-event-translator (12), in-memory-staffing-request (10), decide-budget-change (6), staffing-proposal-slate (5), and other services._
...
- [ ] **DM-5-5** Add missing `updatedAt`; add uniform `createdByPersonId` / `updatedByPersonId` on high-audit tables — BE _partial 2026-05-19 / F-32 (round 6): added on `OrgUnit` + `StaffingRequestProposalSlate`. Combined with F-10.3 + F-17 + F-26 + F-29 + F-30, 12 aggregates now carry the columns. `StaffingRequest` excluded — `requestedByPersonId` is a legacy non-UUID `String` column needing type correction first. Remaining: ~93 lower-audit tables._
...
- [ ] **D-103** — [GAP] Schema-wide actor-audit gap — 0/105 models have `createdById`/`updatedById`. Decide: denormalize on rows (cost M) or formalize "use AuditLog joins" with documented patterns + lint rule. _partial: 12/105 models carry the columns. Rounds: F-10.3 (Project + ProjectAssignment), F-17 (CaseRecord + PersonReleaseRequest), F-26 (LeaveRequest + AssignmentApproval), F-29 (TimesheetWeek + ProjectRisk), F-30 (ProjectMilestone + BudgetApproval), F-32 (OrgUnit + StaffingRequestProposalSlate). Service-layer adoption + full sweep across remaining 93 models continues per-batch._ — `data-quality-audit.md` Part 1 finding 1
