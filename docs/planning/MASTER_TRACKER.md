...
- [ ] **NEW-LGL-1** Re-point live reads to `ProjectPosition.activeFill` — staffing-desk, supply-profile, demand-profile, workforce-planner, dashboards. The bridge from "model built" to "lean flows live." — BE
- [ ] **NEW-LGL-2** SSO admin configuration UI (D-155) — bank Day-1 go-live blocker per `bank-it-deployment-runbook.md`. — FE+BE
- [ ] **NEW-LGL-3** Custom-role admin UI (D-159) — tenant adds bank shapes (Squad/Tribe Lead, IT Service Owner). — FE+BE
...
- [ ] **NEW-LGL-5** Law-7 one-screen unified approvals — position-proposal + leave + activation + skill-review with inline approve/reject. — FE+BE
- [ ] **NEW-LGL-6** Extract `SuggestFillsService` (`suggestForPosition`+`suggestForBatch`) off `ProjectPosition` — matching logic currently embedded in `workforce-planner.autoMatch` over legacy models; backs both planner + single-position Find-Candidates. — BE _(action A-02, Track 3)_
- [ ] **NEW-LGL-7** Build `ProjectPositionDetailPage` (`/projects/:projectId/positions/:positionId`) + `GET /project-positions/:id/candidates` + wire the dark `POST /project-positions/:id/transition` into a reachable flow. V2-H.11 is only a skeleton list; also kills the dead `/positions/:id` cmdk href. — FE+BE _(actions B-01/B-02/B-03, Track 1)_
- [ ] **NEW-LGL-8** Lean `RELEASED` transition replacing dual-approval offboarding — drop-list drops `PersonReleaseRequest`/`PersonReleaseApproval` but nothing builds the replacement. — BE _(action B-06, Track 1)_
...
- [ ] **NEW-LGL-10** Repoint role-dashboard query services + `person-profile` + 3 time-domain reads off InMemory/legacy onto `ProjectPosition`. Verifier found the real legacy load ≈ **34 service files** (V2-H.13's "21" is understated, incl. dashboards/profile/time not enumerated). — BE _(actions A-04/A-05/A-06, Track 5)_
- [ ] **NEW-LGL-11** `GET /leave-requests/preview` (working-days excl. weekend+PublicHoliday, conflicts, balance-after) + `POST /leave-requests/:id/cancel` (self-cancel PENDING). Server-authoritative; LeaveTab drops client-side math. — BE _(actions C-04/C-05, Track 3; overlaps V2-H.9 calc)_
- [ ] **NEW-LGL-12** Emit `leave.approved/rejected/cancelled` outbox + `NotificationEventTranslator` (close **20b-10** — the only OPEN leave loop after 20c-05 closed). — BE _(action C-06, Track 1)_
...
- [ ] **NEW-LGL-17** Subject-employee `AllowSelfScope` read on Cases — onboarding subject sees own case status from `/me`. — BE _(action C-11, Track 3)_
