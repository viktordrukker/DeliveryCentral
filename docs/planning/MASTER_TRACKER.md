...
- [ ] **20c-11** Fix dangerous `as unknown as` type assertions — 12+ instances in cases.controller, hr-dashboard-query, InteractiveOrgChart, SettingsPage. Create proper TypeScript interfaces — BE/FE · Severity: Medium _partial: 19/121 sites closed. F-9.3 / PR #77 closed 7 Decimal-cast sites via `decimalToNumber` helper. F-31 closed 12 sites in `help.service.ts` by switching local row interfaces to `Prisma.<Model>GetPayload<...>` types (drops the coercion at the boundary; 11/11 unit tests still pass). ~109 sites remain across notification-event-translator, in-memory-staffing-request, rate-card-admin, financial-governance services, etc._
