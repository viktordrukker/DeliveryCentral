...
- [ ] **DM-5-5** Add missing `updatedAt`; add uniform `createdByPersonId` / `updatedByPersonId` on high-audit tables — BE _partial 2026-05-18 / F-17 (round 2): added `createdByPersonId` + `updatedByPersonId` on `CaseRecord` + `PersonReleaseRequest` (nullable FK → Person, SetNull). Continues the F-10.3 pattern (Project + ProjectAssignment). 4 aggregates now have the columns. `StaffingRequest` excluded — its `requestedByPersonId` is a legacy non-UUID `String` column needing type correction first. Remaining: ~100 lower-audit tables; per-batch as service-layer adoption catches up._
