...
- [x] **V2-G.4** Flip `dsRefresh` **and `workspaceMe`** defaults to `true` — _2026-05-29 redesign cutover: both flags now default ON (BE registry `platform-flags.service.ts` + FE mirror `feature-flags.ts`), `maturityLevel: 'ga'` (kept in registry for emergency rollback). The new SidebarNavV2/TopHeaderV2 frame, the `/me` workspace, and every DS-adopted surface are now the **default** experience — no per-tenant override needed. Test fallout fixed: the legacy-path suites (`AdminPanelPage`, `EmployeeDetailsPage`, `ProjectDetailsPage`) pinned to flags-OFF (they validate the retained rollback fallback; the ON path is covered by the `.v2*` suites); `route-manifest.test` verified green under flags-ON (39/39). FE tsc + BE build clean._ — BOTH
