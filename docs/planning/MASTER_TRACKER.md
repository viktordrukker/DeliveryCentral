...
| Phase V2 | DS Redesign Operational Cutover | V2-A..V2-H + V2-X | 🔄 In Progress (2026-05-26): **ds-conformance 39→0** + ratchet wired (Husky + CI) + DS Tabs atom shipped + **V2-B.4 SparklineDs CLOSED** + **V2-B.6 VarianceBar CLOSED** + **12 V2-A surfaces CLOSED/partial** (V2-A.1 #371 · V2-A.4 #373 · V2-A.5 #370 · V2-A.6 #370 · V2-A.7 #369 · V2-A.8 #372 · V2-A.10 #375 · V2-A.12 partial #377 · V2-A.14 #360 · V2-A.15 partial #391 · V2-A.17 #389 · V2-A.18 accepted) + **V2-F token cleanup kicked off** (baseline 335→330 via PRs #395/#396). **~46/132 items done (~35%)**. PRs #342–#396 merged or in flight (55+). Atom adoption: Money 2→13, Pct 2→129+, Avatar 6→28, Timeline (DS) 4→5, VarianceBar 1→2. Next: V2-A.3 Plan tab Gantt (depends on workstream BE), continued V2-F long-tail token retirement, continued long-tail atom sweep. |
...
- [-] **V2-F.7** Long-tail (~205 violations across remaining ~73 files) — sweep file-by-file, ratcheting baseline downward. _2026-05-26 kickoff (PRs #395 + #396): baseline ratcheted 335 → 330. Migrated `App.tsx` demo banner, `setup-gate.tsx` loading text, `StatusBadge.tsx` score-variant SVG fill, `AppShell.tsx` demo "Go to Login" button — all migrated to `var(--color-accent)` / `var(--color-surface)` / `var(--color-text-muted)` tokens. Pattern established for incremental file-by-file retirement._ — FE
