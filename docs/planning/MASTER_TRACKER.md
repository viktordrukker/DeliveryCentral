...
- [ ] **D-103** — [GAP] Schema-wide actor-audit gap — 0/105 models have `createdById`/`updatedById`. Decide: denormalize on rows (cost M) or formalize "use AuditLog joins" with documented patterns + lint rule. _partial: 18/105 models carry the columns. Rounds 1-9: F-10.3, F-17, F-26, F-29, F-30, F-32, F-34, F-36, F-38 (NotificationTemplate + ResponsibilityRule — admin-curated config). Remaining ~87 models._ — `data-quality-audit.md` Part 1 finding 1
