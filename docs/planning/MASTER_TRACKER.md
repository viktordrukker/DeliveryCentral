...
- [ ] **20c-09** Create DTOs for 25+ inline `@Body()` parameters — cases controller (addStep, addParticipant, addComment, updateSla), admin controller (updateAccount, importPreview, createWebhook), staffing controller, metadata controller all use unvalidated inline types — BE · Severity: High _partial: Sprint F-9.2 / PR #76 closed 11 of the simpler `@Body() body: { … }` inline-shape sites (reason / runId / wipeFirst patterns). Remaining sites use type-alias patterns (`Omit<ReportTemplate>`, `Partial<HrisConfig>`, `OverridePayload`, `FulfilBody`) and need a second batch._
...
- [ ] **20c-11** Fix dangerous `as unknown as` type assertions — 12+ instances in cases.controller, hr-dashboard-query, InteractiveOrgChart, SettingsPage. Create proper TypeScript interfaces — BE/FE · Severity: Medium _partial: Sprint F-9.3 / PR #77 closed 7 of the `as unknown as { toNumber(): number }` Decimal-cast sites via the new `decimalToNumber` helper. ~114 `as unknown as` sites remain across cases.controller, hr-dashboard-query, InteractiveOrgChart, SettingsPage and other surfaces._
...
- [ ] **D-86** — [DEPRECATE] `/admin/people/new` is alias of `/people/new` (same component, same role gate); add client-side redirect. _audit-2026-05-15: verified 2026-05-15: `frontend/src/app/router.tsx` lines 215-223 render `EmployeeLifecycleAdminPage` for BOTH `/admin/people/new` and `/people/new` — redirect cleanup still legitimate_ — `flow-audit.md` row #2
...
- [ ] **D-89** — [DEPRECATE] Legacy assignment endpoints (`/approve, /reject, /end, /revoke, /activate`) covered by Phase WO-6 already; add `Deprecation` headers as transitional measure. _audit-2026-05-15: partial 2026-05-15: `src/shared/http/deprecated-endpoint.{decorator,interceptor}.ts` infrastructure exists (34 LoC) but zero controller adoption — adding `@Deprecated` to the 5 legacy assignment endpoints is mechanical follow-up_ — `flow-audit.md` row #5; D-04 reference
...
- [ ] **D-96** — [DECIDE] `archivedAt` + `deletedAt` dual soft-delete on `Person` / `Project` / `OrgUnit` — user decision needed: is GDPR purge a real process or vestigial? Cost ranges S→L based on answer. Phase 9 D-167 is the consumer. _audit-2026-05-15: verified 2026-05-15: dual `archivedAt` + `deletedAt` confirmed on Person (`prisma/schema.prisma:446-447`), Project (`:644-645`), ProjectAssignment (`:803-804`) — DECIDE between collapsing or documenting still legitimate_ — register row #6-#8, §3
- [ ] **D-97** — [VERIFY] `Project.projectManagerId` vs `Project.leadPmPersonId` relation duplicate — confirm whether DM-2.5/DM-3 already owns this; if not, audit writers and drop the loser. _audit-2026-05-15: verified 2026-05-15: `Project.projectManagerId` (line 776) and `Project.leadPmPersonId` (line 791) BOTH exist as separate `String? @db.Uuid` FK columns with their own indexes — actual duplicate relation, DM-2.5/DM-3 did not consolidate_ — register row #9, §4
...
- [ ] **D-103** — [GAP] Schema-wide actor-audit gap — 0/105 models have `createdById`/`updatedById`. Decide: denormalize on rows (cost M) or formalize "use AuditLog joins" with documented patterns + lint rule. _partial: Sprint F-10.3 added `createdByPersonId` + `updatedByPersonId` (nullable, FK → persons, SetNull) on the 2 highest-audit aggregates: Project + ProjectAssignment. Service-layer adoption + full sweep across remaining 100+ models follows in F-16.18._ — `data-quality-audit.md` Part 1 finding 1
- [ ] **D-104** — [GAP] `ProjectActivationApproval` (line 177), `PersonReleaseApproval` (line 157), `StaffingRequestFulfilment` (line 2078) missing `createdAt`/`updatedAt`. Add timestamps + Prisma migration. _partial: Sprint F-10.4 added `createdAt` + `updatedAt` on `PersonReleaseApproval` + `StaffingRequestFulfilment`. `ProjectActivationApproval` still pending — has `requestedAt` + `decidedAt` business-time columns; framework-time additions deferred to next batch._ — `data-quality-audit.md` Part 1 finding 2
...
- [ ] **D-117** — [GAP] No `/admin/setup` post-install control surface; setup wizard is one-shot at `/setup`. _audit-2026-05-15: verified 2026-05-15: `/admin/setup` route NOT registered in `route-manifest.ts` / `router.tsx`; only `/setup` (the install wizard, single-use) exists — post-install control surface still legitimate gap_ — `jtbd-validation-matrix.md`
