...
- [ ] **D-103** — [GAP] Schema-wide actor-audit gap — 0/105 models have `createdById`/`updatedById`. Decide: denormalize on rows (cost M) or formalize "use AuditLog joins" with documented patterns + lint rule. _partial: 16/105 models carry the columns. Rounds: F-10.3, F-17, F-26, F-29, F-30, F-32, F-34, F-36 (ProjectBudget + RateCard — financial-grade). Service-layer adoption + sweep across remaining 89 models continues per-batch._ — `data-quality-audit.md` Part 1 finding 1
