...
- [ ] **D-94** — [DROP] Drop orphaned `ProjectTag` and `ProjectTechnology` join tables — zero writes anywhere; `Project.tags`/`techStack` arrays on `Project` are de-facto SoT; supersedes the "consolidate" half of HARDEN_BRIEF D-10. _audit-2026-05-15: verified 2026-05-15: `prisma/schema.prisma:2685` (`ProjectTechnology`) and `:2698` (`ProjectTag`) exist, **zero** `projectTag.` / `projectTechnology.` write references in `src/` outside specs/in-memory — confirmed orphaned, safe to drop_ — `functional-duplication-register.md` row #2-#3, §1
