...
- [ ] **DM-5-5** Add missing `updatedAt`; add uniform `createdByPersonId` / `updatedByPersonId` on high-audit tables — BE _partial 2026-05-19 / F-30 (round 5): added on `ProjectMilestone` + `BudgetApproval`. Combined with F-10.3 + F-17 + F-26 + F-29, 10 aggregates now carry the columns. `StaffingRequest` excluded — `requestedByPersonId` is a legacy non-UUID `String` column needing type correction first. Remaining: ~95 lower-audit tables; per-batch as service-layer adoption catches up._
...
- [ ] **D-103** — [GAP] Schema-wide actor-audit gap — 0/105 models have `createdById`/`updatedById`. Decide: denormalize on rows (cost M) or formalize "use AuditLog joins" with documented patterns + lint rule. _partial: 10/105 models carry the columns. Rounds: F-10.3 (Project + ProjectAssignment), F-17 (CaseRecord + PersonReleaseRequest), F-26 (LeaveRequest + AssignmentApproval), F-29 (TimesheetWeek + ProjectRisk), F-30 (ProjectMilestone + BudgetApproval). Service-layer adoption + full sweep across remaining 95 models continues per-batch._ — `data-quality-audit.md` Part 1 finding 1
