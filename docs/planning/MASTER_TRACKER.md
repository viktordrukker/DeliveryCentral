...
- [ ] **DM-5-5** Add missing `updatedAt`; add uniform `createdByPersonId` / `updatedByPersonId` on high-audit tables — BE _partial 2026-05-18 / F-26 (round 3): added on `LeaveRequest` + `AssignmentApproval`. Combined with F-10.3 (Project + ProjectAssignment) + F-17 (CaseRecord + PersonReleaseRequest), 6 aggregates now have the columns. `StaffingRequest` excluded — its `requestedByPersonId` is a legacy non-UUID `String` column needing type correction first. Remaining: ~100 lower-audit tables; per-batch as service-layer adoption catches up._
...
- [ ] **D-103** — [GAP] Schema-wide actor-audit gap — 0/105 models have `createdById`/`updatedById`. Decide: denormalize on rows (cost M) or formalize "use AuditLog joins" with documented patterns + lint rule. _partial: 6/105 models carry the columns. Sprint F-10.3 (Project + ProjectAssignment), Sprint F-17 (CaseRecord + PersonReleaseRequest), Sprint F-26 (LeaveRequest + AssignmentApproval). Service-layer adoption + full sweep across remaining 100 models continues per-batch._ — `data-quality-audit.md` Part 1 finding 1
