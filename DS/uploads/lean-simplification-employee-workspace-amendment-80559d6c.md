...
| J-9 | Manager / HR / Director | Triage and decide a queue of pending timesheets + leave requests on one surface, with one-screen approval context per item (Law 7) and zero context loss between decisions (Law 3). | `/time-management` (chrome-refreshed; canonical manager-side queue) |
| J-10 | Employee | Submit a clean weekly timesheet in ≤ 90 s with the same visual language as the rest of the workspace — week-at-a-glance Timeline + per-project rows + day cells + submit. | `/timesheets` (chrome-refreshed; reachable as `/me?tab=time&view=week`) |
...
| G-10 | **`/timesheets`, `/timesheets/approval`, and `/time-management` chrome lags the gold-standard.** | Medium | These pages received the 18-E-03 / 18-E-04 standardization pass but predate the `DashboardPage` chrome grammar (KPI strip → hero → action table → secondary analysis) and do not consume the new DS primitives (`Timeline` lifecycle bars, `Calendar`, `BalanceMeter`). Visual density is inconsistent with the staffing-desk amendment and the new `/me/*` workspace, so the manager-side surface reads as "one more table" rather than "this is where time decisions get made." | Chrome refresh on all three: KPI strip + hero `Timeline` (week / queue context) + `DataView` action table + inspector drawer composition. Manager-side `/time-management` becomes the canonical unified queue (Timesheets tab + Leave tab); `/timesheets/approval` becomes a filtered alias of it. Consumes `BalanceMeter` for leave context, `Timeline` for week-at-a-glance. Same visual ethos as the staffing-desk amendment — "this is where decisions get made." |
...
| `/timesheets` | Weekly grid (employee) | **Chrome refresh** | Kept as top-level. Redesigned per §5.1 (S5-H1) onto the gold-standard chrome (KPI strip → hero `Timeline` → entries `DataView`). `/me?tab=time&view=week` is an alias. |
| `/timesheets/approval` | Manager approval queue | **Chrome refresh + alias** | Redesigned per §5.1 (S5-H2). Becomes a filtered alias of `/time-management?type=timesheet&status=pending`; the canonical queue surface is `/time-management`. The old URL keeps working with the new chrome. |
| `/time-management` | Unified approvals (timesheets + leave) | **Chrome refresh — canonical queue** | The flagship manager-side surface. Redesigned per §5.1 (S5-H3). Hosts a Timesheets tab (S5-H2 content) and a Leave tab (containing the Leave Decision Drawer from S5-F7). Same chrome grammar as the staffing-desk planner. |
...
| **S5-H1** | Redesign `TimesheetPage.tsx` onto gold-standard chrome — KPI strip (Filed / Expected / Locked / Overtime), hero `Timeline` (current week, segments by project, lifecycle bars from staffing-desk Timeline upgrade), entries `DataView` (per-project rows × day cells), submit affordance ≤ 200 px from data (Law 4), URL filters (`?week=`, `?view=week|month`) | M | [TimesheetPage.tsx](frontend/src/routes/timesheets/TimesheetPage.tsx), `TimesheetPage.test.tsx`, `frontend/src/components/timesheets/TimesheetWeekHero.tsx` (new) |
| **S5-H2** | Redesign `TimesheetApprovalPage.tsx` queue UX onto Operational Queue grammar — KPI strip (Pending count, Hours pending, Oldest age in days, Approval SLA breach count), queue list, inspector drawer (week grid + anomalies + Approve / Reject with reason); auto-load next on action (Law 3); nudge button row-level integration | M | [TimesheetApprovalPage.tsx](frontend/src/routes/timesheets/TimesheetApprovalPage.tsx), `TimesheetApprovalPage.test.tsx`, `frontend/src/components/timesheets/TimesheetInspectorDrawer.tsx` (new) |
| **S5-H3** | Redesign `TimeManagementPage.tsx` as the canonical manager-side queue — chrome (title bar + tab strip: *Timesheets* / *Leave*) hosting S5-H2 (Timesheets tab) and S5-F7 (Leave tab); shared KPI strip with combined queue metrics; `/timesheets/approval` and `/time-management?type=leave` become deep links into this surface | M | [TimeManagementPage.tsx](frontend/src/routes/time-management/TimeManagementPage.tsx), `TimeManagementPage.test.tsx`, route-manifest |
| **S5-H4** | Period locks + time reports chrome consistency pass — wrap in `PageContainer` + `SectionCard`; replace remaining raw `<table>` with `DataView`; tabular-num on numeric columns; pagination per `dash-action-section__summary` standard | S | [PeriodLocksAdminPage.tsx](frontend/src/routes/admin/PeriodLocksAdminPage.tsx) (if present), `frontend/src/routes/reports/TimeReportPage.tsx`, related tests |
| **S5-H5** | Frontend story + a11y coverage for redesigned surfaces — keyboard navigation through queue + inspector drawer, focus return on close (Law 3), screen-reader landmark labels on tab strip + KPI strip | S | new Storybook stories under `frontend/src/components/timesheets/`, axe-core assertions in the existing test files |
...
| **S5-V3** | E2E Playwright spec: employee submits weekly timesheet on the redesigned `TimesheetPage` → manager opens the redesigned `TimeManagementPage` Timesheets tab → inspector drawer renders with anomalies + week grid → approve → next item auto-loads (Law 3) → notification fires to employee | M | new `playwright/tests/time-management-queue.spec.ts` |
...
### Frontend — manager-side time queue (chrome refresh, S5-H block)

- [TimesheetPage.tsx](frontend/src/routes/timesheets/TimesheetPage.tsx)
- [TimesheetApprovalPage.tsx](frontend/src/routes/timesheets/TimesheetApprovalPage.tsx)
- [TimeManagementPage.tsx](frontend/src/routes/time-management/TimeManagementPage.tsx)
- `frontend/src/components/timesheets/TimesheetWeekHero.tsx` **(new — hero Timeline strip for the weekly grid)**
- `frontend/src/components/timesheets/TimesheetInspectorDrawer.tsx` **(new — manager inspector drawer)**
...
| `/timesheets` | [TimesheetPage.tsx](frontend/src/routes/timesheets/TimesheetPage.tsx) | Weekly grid + day-by-day entries + submit | **Chrome refresh** (S5-H1). Redesigned onto the gold-standard chrome — KPI strip + hero `Timeline` + entries `DataView`. Same content reachable as `/me?tab=time&view=week`. |
| `/timesheets/approval` | [TimesheetApprovalPage.tsx](frontend/src/routes/timesheets/TimesheetApprovalPage.tsx) | Manager approval queue + inspector + auto-next | **Chrome refresh + alias** (S5-H2). Becomes a filtered deep link into `/time-management?type=timesheet&status=pending`; the standalone page renders the new queue UI when hit directly. |
| `/time-management` | [TimeManagementPage.tsx](frontend/src/routes/time-management/TimeManagementPage.tsx) | Unified approvals (timesheets + leave) + monthly compliance matrix | **Chrome refresh — canonical queue surface** (S5-H3). Tab strip hosting *Timesheets* (S5-H2) and *Leave* (S5-F7). Same visual grammar as the staffing-desk planner. |
...
>
> 10. **Employee Timesheet weekly grid redesign** at `/timesheets` (also reachable as `/me?tab=time&view=week`) — Decision Dashboard chrome with: title bar + week navigator (prev / next / "this week") + status chip (DRAFT / SUBMITTED / APPROVED / REJECTED); a **KPI strip** (4 tiles: *Filed (h)*, *Expected (h)*, *Variance*, *Lock status*); a **hero `Timeline`** showing the current week with one swimlane per project + day grid + lifecycle-styled bars (sourced from the staffing-desk Timeline upgrade); the entries `DataView` below (rows = projects, columns = Mon..Sun + Total + Status), with auto-fill button + copy-week popover + submit affordance ≤ 200 px from the data (Law 4). Show one **in-edit state** (active day cell with the inline editor open) and one **submitted-locked state** (cells greyed, status chip green, "Revoke" affordance available only inside the lock window).
>
> 11. **Manager Timesheet approval queue** inside `/time-management?type=timesheet` (and the alias `/timesheets/approval`) — Operational Queue grammar with: title bar + tab strip (*Timesheets* / *Leave*); a **KPI strip** (4 tiles: *Pending count*, *Hours pending*, *Oldest age (days)*, *SLA breach count*); a **queue `DataView`** (compact variant) with columns Person · Week · Hours · Anomalies · Submitted · Age · Status · Actions, sortable by age and SLA breach; an **inspector `Drawer`** that opens on row click showing the full week grid (mini `Timeline` + entries table + anomalies callout + nudge button) and the Approve / Reject (with reason) action ≤ 200 px from the data. On Approve / Reject the drawer auto-loads the next pending item without closing (Law 3). Show one **drawer-open primary state** and one **reject-with-reason state**.
>
> 12. **TimeManagement parent surface — canonical manager-side queue** at `/time-management` — full chrome with: title bar + global filters (range, person, project); the tab strip (*Timesheets* / *Leave*); a **combined KPI strip** (5 tiles: *Pending timesheets*, *Pending leave*, *SLA breach*, *Hours awaiting approval*, *People on leave next 7 d*) — each is a clickable doorway into the corresponding filtered queue (Law 9). The Timesheets tab renders surface #11; the Leave tab renders surface #7 (Leave Decision Drawer). Show the **default landing state** with the Timesheets tab active and the **Leave-tab state** with the Decision Drawer open over a pending request. Same visual ethos as the staffing-desk planner — "this is where time decisions get made."
>
...
> - `frontend/src/routes/timesheets/TimesheetPage.tsx` (existing weekly grid being chrome-refreshed — surface #10)
> - `frontend/src/routes/timesheets/TimesheetApprovalPage.tsx` (existing approval queue being chrome-refreshed — surface #11)
> - `frontend/src/routes/time-management/TimeManagementPage.tsx` (existing manager queue parent surface — chrome refresh / canonical queue — surface #12)
> - `frontend/src/components/staffing-desk/WorkforcePlanner.tsx` (for visual-ethos parity — "this is where decisions get made")
...
>
> 1. PNG mockups (1440 × 900 light + dark) for each of the twelve surfaces
...
> 5. A 1-page "design decisions" doc explaining: workspace tab strip ergonomics (relative to the staffing-desk tab strip — same component or two siblings?), Calendar event-dot vs full-cell-tint decision, BalanceMeter segment-vs-stack visual choice, leave-form preview placement (inline vs side rail), manager Leave Decision Drawer composition, the **inspector-drawer pattern for the timesheet queue (surface #11)** and how it shares atoms with the Leave Decision Drawer (surface #7), the **shared KPI-strip layout across `/me/overview`, `/time-management`, and the staffing-desk planner** (one consistent doorway grammar, Law 9), and any deviations from the existing DS atoms (with rationale).
...
- Workspace chrome conforms to the Decision Dashboard grammar (overview) + List-Detail Workflow grammar (other tabs). Manager-side `/time-management` follows the Operational Queue grammar with a tab strip; `/timesheets` (employee weekly grid) follows the Decision Dashboard grammar.
- Satisfies UX Laws 1, 2, 3, 4, 5, 6, 7, 9, 10. (Manager surfaces add Law 3 — no context loss after approve — via auto-next on the inspector drawer; Law 7 — one-screen approval — via the inspector drawer composition.)
- Employee-first feel: productivity tool, not HR portal. Moderate density.
- **Manager-side feel: same visual ethos as the staffing-desk planner** — premium, opinionated, fast. The KPI strip is the same shape; the inspector drawer is the same atom; the tab strip is the same component. Reading one teaches you the others.
- Back-compat: every legacy route still works (alias-and-redirect, plus chrome refresh for `/timesheets`, `/timesheets/approval`, `/time-management`).
